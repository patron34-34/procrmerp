...
import { LeaveStatus, LeaveType, Payslip } from '../../../types';
import PayslipDetailModal from '../../hr/PayslipDetailModal';

type ActiveTab = 'ozluk' | 'leaves' | 'performance' | 'payroll';
...
const EmployeeDetail: React.FC<EmployeeDetailProps> = ({ employeeId: propEmployeeId }) => {
    ...
    const { employees, leaveRequests, performanceReviews, payslips } = useApp();
                    ...
                    <Card title="İzin Geçmişi">
                        <table className="w-full text-left">
                            <thead><tr className="bg-slate-50 dark:bg-slate-900/50">
                                <th className="p-2">Tür</th><th className="p-2">Başlangıç</th><th className="p-2">Bitiş</th><th className="p-2">Durum</th>
                            </tr></thead>
                            <tbody>{employeeLeaves.map(lr => (
                                <tr key={lr.id} className="border-b dark:border-dark-border">
                                    <td className="p-2">{lr.leaveType}</td>
                                    <td className="p-2">{lr.startDate}</td>
                                    <td className="p-2">{lr.endDate}</td>
                                    <td className="p-2">{getLeaveStatusBadge(lr.status)}</td>
                                </tr>
                            ))}</tbody>
                        </table>
                    </Card>
                ...
                return (
                    <Card title="Performans Değerlendirmeleri">
                        {employeeReviews.map(review => (
                            <div key={review.id} className="p-3 border rounded-md mb-3 dark:border-dark-border">
                                <p className="font-bold">Dönem: {review.periodStartDate} - {review.periodEndDate}</p>
                                <p><strong>Puan:</strong> {review.overallRating}/5</p>
                                <p><strong>Değerlendiren:</strong> {review.reviewerName}</p>
                            </div>
                        ))}
                    </Card>
                );
            case 'payroll':
                 return (
                    <Card title="Maaş Pusulaları">
                        <table className="w-full text-left">
                            <thead><tr className="bg-slate-50 dark:bg-slate-900/50">
                                <th className="p-2">Dönem</th><th className="p-2 text-right">Brüt Maaş</th><th className="p-2 text-right">Net Maaş</th><th className="p-2 text-center">Eylemler</th>
                            </tr></thead>
                            <tbody>{employeePayslips.map(p => (
                                <tr key={p.id} className="border-b dark:border-dark-border">
                                    <td className="p-2 font-medium">{p.payPeriod}</td>
                                    <td className="p-2 text-right font-mono">${p.grossPay.toLocaleString()}</td>
                                    <td className="p-2 text-right font-mono text-green-600 font-semibold">${p.netPay.toLocaleString()}</td>
                                    <td className="p-2 text-center">
                                        <button onClick={() => setViewingPayslip(p)} className="text-primary-600 hover:underline text-sm">Görüntüle</button>
                                    </td>
                                </tr>
                            ))}</tbody>
                        </table>
