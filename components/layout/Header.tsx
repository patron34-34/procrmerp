...
const Header: React.FC = () => {
  ...
  const getPageTitle = () => {
    ...
    const pageTitles: { [key: string]: string } = {
        ...
        '/finance/bills': 'Gider Faturaları',
        '/finance/budgets': 'Bütçeler',
        '/finance/bank-reconciliation': 'Banka Mutabakatı',
  ...
  return (
    <header className="h-16 bg-sidebar dark:bg-dark-sidebar border-b border-border dark:border-dark-border flex items-center justify-between px-6 lg:px-8 shrink-0">
      <div>
        ...
        <div className="relative" ref={userMenuRef}>
            <button onClick={() => setIsUserMenuOpen(!isUserMenuOpen)} className="flex items-center">
              <div className="text-right hidden sm:block">
                  <span className="font-semibold block text-sm text-text-main dark:text-dark-text-main">{currentUser.name}</span>
            ...
            {isUserMenuOpen && (
                <div className="absolute right-0 mt-2 w-56 bg-card rounded-lg shadow-lg z-20 border border-border dark:bg-dark-card dark:border-dark-border">
                    <div className="p-2">
                        <Link to="/profile" onClick={() => setIsUserMenuOpen(false)} className="w-full text-left flex items-center gap-3 px-2 py-2 text-sm text-text-secondary hover:bg-slate-100 dark:text-dark-text-secondary dark:hover:bg-dark-border rounded-md">
                           Profilim
                        </Link>
                    </div>
                    ...
                    <ul className="max-h-60 overflow-y-auto p-2">
                        {employees.map(user => (
                            <li key={user.id}>
                                <button onClick={() => handleUserChange(user)} className="w-full text-left flex items-center gap-3 px-2 py-2 text-sm text-text-secondary hover:bg-slate-100 dark:text-dark-text-secondary dark:hover:bg-dark-border rounded-md">
